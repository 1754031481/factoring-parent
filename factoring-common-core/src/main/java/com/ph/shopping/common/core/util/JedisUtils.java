...
public class JedisUtils {
	   /* private static final Logger LOGGER = Logger.getLogger(JedisUtils.class);*/
		...
		public static JedisUtils getRu(InitParams initParams) {
			if (pool == null) {
	    		/*String ip = InitListener.getValue("redis.ip", "192.168.116.207");
	    		int port = Integer.parseInt(InitListener.getValue("redis.port", "5379"));*/
	            JedisPoolConfig config = new JedisPoolConfig();
	            ...
	            //pool = new JedisPool(config, "10.141.111.185", 6379, 100000);
	           //InitParamPc initParam = new InitParamPc();
	            String redisHost = initParams.getProperties().getProperty("spring.redis.host");
	            Integer redisPort = Integer.valueOf(initParams.getProperties().getProperty("spring.redis.port"));
	            pool = new JedisPool(config, redisHost, redisPort, 100000);
	        }
			return ru;
		}
