...
import com.ph.shopping.facade.permission.spmEnum.SPMEnum;
...
public class UserServiceImpl implements IUserService {
    ...
    @Override
    public Result getUserByPageDP(PageBean page,UserDTO userDto) {
        PageHelper.startPage(page.getPageNum(), page.getPageSize());
        List<UserVO> list = userMapper.getUserByPageDP(userDto);
        PageInfo<UserVO> pageInfo = new PageInfo<>(list);
        return ResultUtil.getResult(PermissionEnum.Code.SUCCESS,pageInfo.getList(),pageInfo.getTotal());
    }
    ...
    public Result resetPassword(User user, SmsSourceEnum smsSourceEnum, String type) {
        try {
            ...
            String password1=MD5.getMD5Str("e10adc3949ba59abbe56e057f20f883e");
    ...
    public Result checkUserByCondition(UserDTO userDto) {
        ...
        if (userVO == null) {
            ...
            if (userVO.getAgentPass() != null && !(SPMEnum.agentStatus.ONE.getIndex() == userVO.getAgentPass())) {
                ...
                return ResultUtil.getResult(PermissionEnum.LOCKED_ACCOUNT);
            } else if (two.equals(userVO.getIsable())) {
                return ResultUtil.getResult(PermissionEnum.LOCKED_ACCOUNT);
            }
    ...
    @Override
    public Result updateSuppPassword(Map<String, Object> map) {
        Result result = new Result();
        result.setCode("0");
        try {
            roleMapper.updateSuppPassword(map);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("重置密码异常", e);
            result.setCode("0");
