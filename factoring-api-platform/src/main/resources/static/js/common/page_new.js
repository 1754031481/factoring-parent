(function($){
	var zp = {
		...
		addhtml:function(obj,pageinit){
			return (function(){
				obj.empty();
				// alert(pageinit.shownum+"====="+pageinit.current+"======"+pageinit.pageNum);
				...
				for (;start <= end;start++) {
					if (start <= pageinit.pageNum && start >=1) {
						// alert(pageinit.current+"=====start:"+start+"====end:"+end+"====pageNum:"+pageinit.pageNum);
		...
		bindEvent:function(obj,pageinit){
			return (function(){
				...
				obj.on("click","a.dataPagenum",function(){
					...
					zp.addhtml(obj,current);
					if (typeof(pageinit.backfun)=="function") {
						// alert("dataPagenum"+current.current);
						pageinit.backfun(current);
					}
				});
